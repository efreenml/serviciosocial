...
if(isset($_POST)){
	...
	$permisos = isset($_POST['permisos']) ? mysqli_real_escape_string($db, trim($_POST['permisos'])) : false;
	...
	if(count($errores) == 0){
		...
		if($isset_user['id'] == $usuario['id'] || $usuario['rolUsuario'] == "administrador"){
			...
			if($password != ""){
				...
				$sql .= " ,password = '$password_segura' ";
			}
			if($permisos != ""){
				$sql .= " ,rolUsuario = '$permisos' ";
			}
